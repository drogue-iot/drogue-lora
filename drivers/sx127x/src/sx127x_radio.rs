...
use super::sx127x_lora::{Error as PhyError, LoRa, RadioMode, IRQ};
        ...
        match event {
            LoraEvent::TxRequest(config, buf) => {
                ...
                match result {
                    ...
                    Err(e) => (State::Idle, Err(LoraError::PhyError(()))),
                }
            }
    ...
    pub fn handle_event_txing(
        ...
        match event {
            LoraEvent::PhyEvent(phyevent) => match phyevent {
                ...
                _ => (State::Txing, Ok(LoraResponse::Txing)),
            },
    ...
    pub fn handle_event_rxing(
        ...
        match event {
            LoraEvent::PhyEvent(phyevent) => match phyevent {
                ...
                _ => (State::Rxing, Ok(LoraResponse::Rxing)),
            },
            LoraEvent::CancelRx => {
                self.radio.set_mode(RadioMode::Sleep).ok().unwrap();
